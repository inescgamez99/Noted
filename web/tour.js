...
// Returns the string for the current UI language (es/en/ca)
function d(es, en, ca) {
...
function injectDemoMeeting() {
  ...
  if (list) {
    list.innerHTML = `
      <div class="day-group">
        <div class="day-label">${d('Hoy', 'Today', 'Avui')}</div>
  ...
  panel.innerHTML = `
    <div class="meeting-detail">
      <div class="detail-header">
        <div>
          <div class="detail-title-row">
            ...
            <button class="btn-rename-meeting" id="btn-rename-meeting" title="${d('Editar','Edit','Editar')}">${_SVG_EDIT}</button>
          </div>
          <div class="detail-meta">
            <span>23 Sep 2026 · 10:00</span>
            <span class="detail-project-wrap">
              <label class="detail-project-label">${d('Proyecto','Project','Projecte')}</label>
              <select class="detail-project-select" id="meeting-project-select">
                <option>${d('— Sin proyecto —','— No project —','— Sense projecte —')}</option>
        ...
        <div class="detail-actions-bar" id="tour-detail-actions-bar">
          ...
          <button class="action-icon-btn" id="btn-edit-notes" title="${d('Editar','Edit','Editar')}">${_SVG_EDIT}</button>
          <button class="action-icon-btn" id="btn-copy" title="${d('Copiar','Copy','Copiar')}">${_SVG_COPY}</button>
          <button class="action-icon-btn" id="btn-email" title="Email">${_SVG_EMAIL}</button>
          <button class="action-icon-btn" id="btn-sticky-bar" title="${d('Nota adhesiva','Sticky note','Nota adhesiva')}">${_SVG_STICKY}</button>
          <button class="action-icon-btn" id="btn-regenerate" title="${d('Regenerar minutas','Regenerate notes','Regenerar notes')}">${_SVG_REGEN}</button>
          <div class="action-more-wrap">
            ...
            <div class="action-menu hidden" id="action-menu">
              <button class="action-menu-item" id="btn-sticky">${_SVG_STICKY}<span>${d('Nota adhesiva','Sticky note','Nota adhesiva')}</span></button>
      ...
      <div class="regen-bar hidden" id="regen-bar">
        <textarea id="regen-textarea" rows="2" placeholder="${d('Describe qué cambios quieres en las minutas...','Describe the changes you want in the notes...','Descriu els canvis que vols a les notes...')}"></textarea>
        <div class="regen-bar-btns">
          <button class="btn btn-primary btn-sm" id="btn-regen-confirm">${d('Regenerar','Regenerate','Regenerar')}</button>
          <button class="btn btn-ghost btn-sm" id="btn-regen-cancel">${d('Cancelar','Cancel','Cancel·lar')}</button>
        </div>
      </div>
      <div class="detail-tabs">
        <button class="detail-tab active" id="tab-notes" data-tab="notes">${d('Notas','Notes','Notes')}</button>
        <button class="detail-tab" id="tab-actions" data-tab="actions">${d('Acciones','Actions','Accions')} <span class="tab-badge">4</span></button>
      ...
      <div class="minutes-section" id="section-notes">
        <div class="minutes-content">
          <h2>${d('Resumen','Summary','Resum')}</h2>
          <p>${d(
            'Reunión de planificación del Q4 con el equipo de tecnología de Accenture. Se revisaron los objetivos del trimestre, se asignaron responsabilidades y se acordó el calendario de entregas para los próximos tres meses.',
            'Q4 planning meeting with the Accenture Technology team. Quarterly objectives were reviewed, responsibilities assigned, and the delivery calendar for the next three months agreed upon.',
            'Reunió de planificació del Q4 amb l\'equip de tecnologia d\'Accenture. Es van revisar els objectius del trimestre, s\'assignaren responsabilitats i es va acordar el calendari de lliuraments pels propers tres mesos.'
          )}</p>
          <h2>${d('Decisiones','Decisions','Decisions')}</h2>
          <ul>
            <li>${d('Usar Noted como herramienta estándar de documentación en el equipo','Use Noted as the team\'s standard documentation tool','Usar Noted com a eina estàndard de documentació a l\'equip')}</li>
            <li>${d('Configurar reunión de seguimiento semanal los lunes a las 9:00','Set up weekly follow-up meeting on Mondays at 9:00','Configurar reunió de seguiment setmanal els dilluns a les 9:00')}</li>
            <li>${d('Priorizar la integración con repositorios antes de fin de octubre','Prioritise repository integration before end of October','Prioritzar la integració amb repositoris abans de finals d\'octubre')}</li>
          </ul>
          <h2>${d('Acciones','Actions','Accions')}</h2>
          <ul>
            <li>📌 <strong>Ines</strong> — ${d('Distribuir Noted al resto del equipo antes del viernes','Share Noted with the rest of the team before Friday','Distribuir Noted a la resta de l\'equip abans del divendres')}</li>
            <li>📌 <strong>Felipe</strong> — ${d('Revisar y cerrar los PRs pendientes esta semana','Review and close pending PRs this week','Revisar i tancar els PRs pendents aquesta setmana')}</li>
            <li>📌 <strong>Ana y Maria</strong> — ${d('Preparar demo de nuevas funcionalidades para el cliente','Prepare new features demo for the client','Preparar demo de noves funcionalitats per al client')}</li>
            <li>📌 <strong>${d('Todo el equipo','Whole team','Tot l\'equip')}</strong> — ${d('Completar la formación de onboarding antes del 30 de septiembre','Complete onboarding training before 30 September','Completar la formació d\'onboarding abans del 30 de setembre')}</li>
          </ul>
        </div>
      </div>
      <div class="actions-section hidden" id="section-actions">
        <div class="actions-section-header">
          <div class="section-label">${d('Acciones','Actions','Accions')}</div>
          <div class="actions-count" style="margin-left:auto">${d('4 en total','4 total','4 en total')}</div>
          <button class="btn btn-ghost btn-sm" style="margin-left:8px">${d('+ Añadir','+ Add','+ Afegir')}</button>
        </div>
        <div id="meeting-actions">
          <div class="action-card" id="card-0">
            <div class="action-card-row">
              <div class="action-card-main">
                <span class="action-title">${d('Distribuir Noted al resto del equipo','Share Noted with the rest of the team','Distribuir Noted a la resta de l\'equip')}</span>
                <div class="action-meta"><span class="action-assignee">Ines</span> · <span style="font-size:11px;color:var(--muted)">${d('viernes','Friday','divendres')}</span></div>
              </div>
              <div class="action-card-btns">
                <button class="btn btn-ghost btn-sm" id="tour-move-panel-btn">${d('Mover al panel','Move to board','Moure al tauler')}</button>
          ...
          <div class="action-card" id="card-1">
            <div class="action-card-row">
              <div class="action-card-main">
                <span class="action-title">${d('Revisar y cerrar los PRs pendientes','Review and close pending PRs','Revisar i tancar els PRs pendents')}</span>
                <div class="action-meta"><span class="action-assignee">Felipe</span></div>
              </div>
              <div class="action-card-btns">
                <button class="btn btn-ghost btn-sm">${d('Mover al panel','Move to board','Moure al tauler')}</button>
                <button class="btn btn-delete btn-sm">×</button>
              </div>
            </div>
          </div>
          <div class="action-card" id="card-2">
            <div class="action-card-row">
              <div class="action-card-main">
                <span class="action-title">${d('Preparar demo de nuevas funcionalidades para el cliente','Prepare new features demo for the client','Preparar demo de noves funcionalitats per al client')}</span>
          ...
          <div class="action-card" id="card-3">
            <div class="action-card-row">
              <div class="action-card-main">
                <span class="action-title">${d('Completar la formación de onboarding','Complete onboarding training','Completar la formació d\'onboarding')}</span>
                <div class="action-meta"><span class="action-assignee">${d('Todo el equipo','Whole team','Tot l\'equip')}</span> · <span style="font-size:11px;color:var(--muted)">30 sep</span></div>
              </div>
              <div class="action-card-btns">
                <button class="btn btn-ghost btn-sm">${d('Mover al panel','Move to board','Moure al tauler')}</button>
                <button class="btn btn-delete btn-sm">×</button>
      ...
      <div class="transcript-section hidden" id="section-transcript">
        <div class="transcript-content" style="color:var(--muted);font-style:italic;padding:24px;text-align:center">${d('Transcript de la reunión de demo no disponible.','Demo meeting transcript not available.','Transcript de la reunió de demo no disponible.')}</div>
      </div>
...
function injectDemoTasks() {
  ...
  body.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:8px;padding:12px">
      <div style="font-size:11px;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.05em;padding:4px 0">${d('En curso','In progress','En curs')}</div>
      ${_taskRow('tour-task-1', d('Ponerse al día con Dani sobre el estado del proyecto','Catch up with Dani on project status','Posar-se al dia amb Dani sobre l\'estat del projecte'), 'Dani')}
      ${_taskRow('tour-task-2', d('Organizar con Sara y el LS Hub team la siguiente reunión de HBA','Organise the next HBA meeting with Sara and the LS Hub team','Organitzar amb Sara i l\'LS Hub team la propera reunió de HBA'), 'Sara')}
      ${_taskRow('tour-task-3', d('Agendar sesión con Ricardo para explorar sus ideas de add-ons para Noted','Schedule a session with Ricardo to explore his add-on ideas for Noted','Agendar sessió amb Ricardo per explorar les seves idees d\'add-ons per a Noted'), 'Ricardo')}
      <div style="font-size:11px;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.05em;padding:12px 0 4px">${d('Pendiente','Pending','Pendent')}</div>
      ${_taskRow('tour-task-4', d('Preguntar a Jaime por opciones de restaurante para la cena del equipo del jueves','Ask Jaime for restaurant recommendations for Thursday\'s team dinner','Preguntar a Jaime per opcions de restaurant per al sopar de l\'equip del dijous'), 'Jaime', d('Jueves','Thursday','Dijous'))}
      ${_taskRow('tour-task-5', d('Alinear con Marta Bailador y Elsa la disponibilidad del equipo para Q4','Align with Marta Bailador and Elsa on team availability for Q4','Alinear amb Marta Bailador i Elsa la disponibilitat de l\'equip per al Q4'), 'Marta B.')}
      ${_taskRow('tour-task-6', d('Pedir a Guille que prepare una demo del caso Pfizer para el equipo','Ask Guille to prepare a Pfizer case demo for the team','Demanar a Guille que prepari una demo del cas Pfizer per a l\'equip'), 'Guille')}
      ${_taskRow('tour-task-7', d('Hablar con Feli para incluir Noted en el repositorio compartido del equipo','Talk to Feli about adding Noted to the team\'s shared repo','Parlar amb Feli per incloure Noted al repositori compartit de l\'equip'), 'Feli')}
      ${_taskRow('tour-task-8', d('Robarle la bolsa de Trader Joe\'s a Miguel antes de que se la lleve a casa','Steal Miguel\'s Trader Joe\'s bag before he takes it home','Robar la bossa de Trader Joe\'s al Miguel abans que se la porti a casa'), 'Miguel')}
    </div>`;
}
    ...
    <div class="project-settings-item" style="border-left-color:#A100FF">
      <div class="project-settings-row" style="cursor:default;pointer-events:none">
        <div style="min-width:0;flex:1">
          ...
          <div class="project-settings-desc">${d(
            'Iniciativas internas del equipo: herramientas de productividad, demos con clientes y formación en IA.',
            'Internal team initiatives: productivity tools, client demos and AI training.',
            'Iniciatives internes de l\'equip: eines de productivitat, demos amb clients i formació en IA.'
          )}</div>
        </div>
